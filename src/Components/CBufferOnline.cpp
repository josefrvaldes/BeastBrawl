 ...
 */
void CBufferOnline::InsertNewReceivedOnline(int64_t time, vector<Constants::InputTypes> inputs) {
    BuffElement buffElem(time, inputs);
    if(elems.empty()) {
 ...
 * Esta función recibe un elemento nuevo y lo inserta donde corresponda manteniendo el orden por tiempo. Además, elimina todos los anteriores que hubiera por tiempo.
 */
void CBufferOnline::InsertNewCalculated(vec3 pos_, vec3 rot_) {
    BuffElement buffElem(pos_, rot_);
    elems.push_back(buffElem);
}