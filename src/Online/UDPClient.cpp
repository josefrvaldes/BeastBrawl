    ...
    if (!errorCode) {
        ...
        switch (callType) {
            case Constants::PetitionTypes::SEND_INPUTS: {
                uint16_t idInputCall;
                uint16_t idRival;
                Utils::Deserialize(&idInputCall, recevBuff.get(), currentIndex);
                ...
                if (idInputCall > lastInputIDReceived) {
                    lastInputIDReceived = idInputCall;
